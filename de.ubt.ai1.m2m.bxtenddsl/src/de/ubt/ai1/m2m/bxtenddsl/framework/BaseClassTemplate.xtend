 ...
 */
final class BaseClassTemplate {
	/**
	 * Returns Xtend source code for the abstract base rule class.
	 */
	def static String getCode(FrameworkConfig config) {
		'''
package «config.rootPackage».rules

import «config.rootPackage».corrmodel.Corr
import «config.rootPackage».corrmodel.CorrElem
import «config.rootPackage».corrmodel.CorrModelFactory
import «config.rootPackage».corrmodel.MultiElem
import «config.rootPackage».corrmodel.SingleElem
import «config.rootPackage».corrmodel.Transformation
import «config.rootPackage».trafo.«config.trafo»
...
abstract class Elem2Elem {
	...
	val protected «config.trafo» trafo
	...
	val static sourcePackage = «config.sourcePackage»::eINSTANCE
	val static targetPackage = «config.targetPackage»::eINSTANCE
	val static sourceFactory = «config.sourceFactory»::eINSTANCE
	val static targetFactory = «config.targetFactory»::eINSTANCE
	val static corrFactory = CorrModelFactory::eINSTANCE
	
	static Map<EObject, Corr> elementsToCorr = newHashMap()
	
	new(String ruleId, «config.trafo» trafo) {
		this.ruleId = ruleId
		this.trafo = trafo
